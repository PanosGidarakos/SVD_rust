...
if __name__ == '__main__':
   ...
   cat = data.chelsea()
    ...
    fig, axes = plt.subplots(3, len(r_values) + 1, figsize=(15, 6))  # Adjust figure size as needed
    
    # Compute SVDs
    u_g_r, s_g_r, v_g_r = g_r_svd(gray_cat.copy())
    singularValues_n_e, us_n_e, vs_n_e = n_e_svd(gray_cat.copy())
    u,b,v=numpy_svd(gray_cat.copy())
    ...
    # Plot the original grayscale image again in the second row for consistency
    axes[2, 0].imshow(gray_cat, cmap='gray')
    axes[2, 0].set_title('Original')
    axes[2, 0].axis('off')
    
    # Plot the n_e_svd approximations in the second row
    for i, r in enumerate(r_values):
        approximation = u[:, :r] @ np.diag(b)[:r, :r] @ v[:r, :]
        axes[2, i + 1].imshow(approximation, cmap='gray')
        axes[2, i + 1].set_title(f'Numpy_SVD k={r}')
        axes[2, i + 1].axis('off')
    plt.tight_layout()
